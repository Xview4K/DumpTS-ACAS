...
本バージョンはVS2022 Win32(x86)でビルドテスト済み。
暗号ライブラリはcryptopp(ソース)とOpenSSLのWindowsビルド済みバージョンを使用。
https://cryptopp.com/release890.html
cryptoppのソースをWin32(x86)用に自分でビルドする。

https://slproweb.com/products/Win32OpenSSL.html
Win32(x86) OpenSSL v3.2.2や3.2.1の202MB Installerをダウンロード
x64でビルドする場合は、Win64を選択して、ソリューションのライブラリのパスをインストールしたフォルダに設定。

リンカー→追加ファイルにwinscard.lib;libssl.lib;libcrypto.lib;cryptlib.lib;を記述。

# DumpTS ACAS処理の概要
DumpMMT.cppとMMT.hでMMT/TLVストリームをパケット単位に分解。
主要なパケットはpHeaderCompressedIPPacketで、ここに映像・音声データ、暗号解除に必要なECMが含まれる。
ECMデータをa_cas_card.cでACAS ICカードに渡し、得られる暗号解除キーを res.scramble_key[AES_KEY_SIZE]に保存。
映像・音声データはscramble_keyを用いてCrypt++で暗号解除する。
（正常に暗号解除されていれば出力out.data()の先頭部分は0x00）

暗号解除した映像・音声データをProcessMFUに入力して、一般的なソフトで再生できる動画ストリームを得る。

MMT.hでptr_MPU、ptr_encrypted_MPU（暗号解除前の映像・音声データ）、ptr_decrypted_MPU（暗号解除後の映像・音声データ）は同じメモリアドレス(union)なので、暗号化部分以外のデータは暗号解除前後で再処理せずに値を参照できる。
